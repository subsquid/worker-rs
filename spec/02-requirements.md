...
| OQ-8 | The network-state document's `effective_from` is honored by portals (fleet-coordinated cutover) but ignored by the worker (IB-40): during the window the worker may already reshuffle chunks that portals still route to under the previous assignment. Should WP-1 delay application until the effective time? The split pointer carries no `effective_from` at all (IB-40b), so the question is legacy-only — and the split shape answers it by removing the field rather than by deciding it | WP-1, IB-40 | network/scheduler team |
