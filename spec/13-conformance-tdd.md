...
| FM-53c | CT-4 | P | unit-tested: an assignment naming a schema the bundle lacks is refused whole, and a schema survives a later bundle that omits it (bundles merge — IB-44b). ADR-21's stricter rule is driven through the harness: a bundle that omits the schema its assignment references leaves the assignment unapplied and nothing fetched, and the test fails if coverage is judged against the accumulated store instead. The OB-16 alarm is unasserted |
