...
| FM-2..3 | CT-4 | U | |
| FM-10 | CT-4 | P | the retry itself is unmeasured, but a corrected fetch location under an unchanged pair reaching a stalled retry is asserted at both ends — the poll announces the move, the applier replaces the queued copy — and a location moving under an applied pair re-fetches nothing |
...
| FM-12 | CT-4 | P⊘ | a document that cannot be read is refused rather than fatal, asserted at the applier with a corrupted roster; a roster naming a table that is not a file name is refused too, so a document cannot write a chunk's data outside its directory and still commit the chunk; oversize intake is still unbounded (GAP-4) |
...
| FM-53d | CT-4 | P | unit-tested at the poll: a state with an assignment and no bundle is answered as not-applicable rather than an error, the pair fault is counted, and the announced pair is left alone so the bundle's return re-offers it whole. That erroring instead costs hours of backoff is reasoned, not driven — no test measures the poll cadence |
