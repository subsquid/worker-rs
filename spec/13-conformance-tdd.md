...
| FM-53c | CT-4 | P | unit-tested: an assignment naming a schema the bundle lacks is refused whole, and a schema survives a later bundle that omits it (bundles merge — IB-44b). ADR-21's stricter rule is enforced (coverage is judged against the bundle in force, not the accumulated store) and alarms via OB-16, but no test asserts either. Not driven through the harness |
