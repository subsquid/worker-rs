...
| FM-11 | document with a malformed per-chunk entry (a file address that will not build for one chunk) | degrade: the affected chunk fails on its own (WP-2), at once rather than over its retry budget, since a document does not change between attempts; rest of the document applies; alarm (OB-17). Credentials are per worker in both formats, not per chunk, so one that won't decrypt is FM-12's whole-document rejection. So is everything the document says about itself at dataset level — a base url that will not parse, a version the dataset registers no generation for, a chunk naming a write schema the document's own roster table does not define: that is the document contradicting itself, not naming an external resource wrongly, and nothing about the rest of it can be trusted more than that part. A chunk the document addresses correctly but the origin will not serve is not a document fault at all: it is retried within its budget (WP-13) and the next assignment restores the budget |
| FM-12 | document malformed as a whole, oversized, undecodable, or missing this worker; inconsistent with itself — a dataset base url that will not parse, a version with no generation entry, a write schema with no roster; inconsistent with the store it is written to — a roster table that is not a file name or too long for one; or the network state's pointer to it will not decode or names no fetch url | fail-safe + alarm (OB-18): reject whole document (WP-2); keep prior assignment; keep serving — including a document that cannot be read at all, whose reader panic is contained where it happens rather than forwarded. An unusable pointer is refused at the poll and re-read at the poll cadence, not the fetch-retry ladder (WP-1): the state was read, its content is not applicable. [Oversize unbounded today — GAP-4] |
...
| FM-53 | schema-registry unreachable or malformed manifest (IB-44), while an assignment that resolves by type is in force (under `split` the manifest is not polled) | degrade: keep previously loaded schemas. The manifest is never stored (ADR-22), so a restart starts empty: until the first load lands, a dynamic-engine query against a chunk the assignment in force does not pin fails typed `server_error`. Chunks it does pin read their schema by id from the store and are unaffected (IB-44b) |
| FM-53b | schema bundle unreachable, hash mismatch, or local I/O while staging — or, past the hash check, unusable: no `<id>.yaml` entry, not one schema that will parse (IB-44b, a split assignment; an id republished with different contents is not a fault — it replaces the stored copy) | fail-safe: the jointly validated update cannot complete, so the assignment is not applied and the worker keeps serving the previous one. Valid schemas cached by an interrupted earlier attempt may remain. Unreachable, mismatched or failing locally: retried with backoff, and a corrected location rescues it (IB-40b); a bundle that fails to install is re-offered rather than recorded as consumed. Unusable past the hash check: the bytes are the ones the hash vouched for, so it is a verdict on the pair — refused once like FM-12 (OB-18), not fetched again until a different pair is announced; a store repaired locally is picked up on the next pair or at restart. Observable via OB-16 |
...
| FM-53d | network state resolves to a `split` assignment without a usable schema bundle — the bundle missing, or its hash not `sha256:<hex>` (IB-40b) | fail-safe + alarm: the state parsed, it is simply not applicable — apply nothing, keep serving the previous assignment, and re-read at the poll interval rather than on the fetch-retry ladder, since an incomplete state is a legal condition of a rolling migration and not a failure to read one. Alarms as a scheduler fault (OB-16). How long it has persisted is not signalled — that is OB-13, which does not exist (GAP-23) |
| FM-53e | network state resolves to a type whose remaining blobs it does not publish — a `split` state without `portal_assignment` or `worker_assignment`, a `legacy` one without `assignment`, or an `assignment_type` the worker cannot read with none pinned (IB-40/40b) | fail-safe: not applicable, so apply nothing, keep serving the previous assignment, and re-read at the poll interval like FM-53d. Counted by reason at every poll (OB-19): a brief absence is a migration in flight and nothing to act on, but it is the same shape a scheduler leaves when it declares a type it has not finished publishing — which stalls every worker — so what has to be visible is how long it lasts, not that it happened |
