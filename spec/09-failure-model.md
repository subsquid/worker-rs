...
| FM-53b | schema bundle unreachable, hash mismatch, or unusable (IB-44b, `USE_WORKER_ASSIGNMENTS`) | fail-safe, *not* a degrade: the bundle is a prerequisite for the assignment it accompanies, so the assignment is not applied and the worker keeps serving the previous one. Retried with backoff; a bundle that fails to install is re-offered on the next poll rather than recorded as seen. Deliberate — the flag is opt-in, and a broken bundle should surface during rollout rather than when something first depends on it. Observable via `schema_bundle_loaded` / `schema_bundle_failures` |
| FM-53c | assignment references a write schema the loaded bundle doesn't carry (IB-41b) | fail-safe: refuse the assignment whole, keep serving the previous one. Applying it would download chunks that cannot be queried, and under a type-keyed fallback would silently execute them against a different version of the same dataset type. A bundle replacement never drops a schema the assignment in force still uses — those are carried over and the omission logged |
