...
#[instrument(skip_all)]
async fn download_dir(files: Vec<RemoteFile>, dst_dir: PathBuf) -> Result<()> {
    ...
    futures::future::try_join_all(files.into_iter().map(|file| async move {
        ...
        download_one(&file.url, &dst_file).await
    }))
    .await?;
...
#[instrument(skip_all)]
pub async fn download_one(url: &str, dst_path: &Path) -> Result<()> {
