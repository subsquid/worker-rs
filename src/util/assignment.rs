...
impl Assignment {
    ...
    pub fn dataset_chunks_for_peer_id(&self, peer_id: String) -> Option<Vec<Dataset>> {
        ...
        let flat_list: Vec<(&Dataset, &Chunk)> = self.datasets
            .iter()
            .flat_map(|dataset| {
                dataset
                    .chunks
            ...
            .collect();
        let mut result: HashMap<String, Dataset> = Default::default();
        for index in idxs {
            let (dataset, chunk) = flat_list.get(index as usize)?;
            result.entry(dataset.id.clone()).or_insert_with(|| Dataset {
                id: dataset.id.clone(),
                base_url: dataset.base_url.clone(),
                chunks: vec![],
            }).chunks.push((*chunk).clone());
        }
        Some(result.into_values().collect())
    ...
    pub fn chunk_index(&mut self, chunk_id: String) -> Option<u64> {
        ...
        self.chunk_map.as_ref().unwrap().get(&chunk_id).copied()
    }
